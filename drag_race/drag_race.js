...
const addBestPlayerToLocalStorage = () => {
  const players = Object.values(sessionStorage).map((player) =>
    JSON.parse(player)
  );

  const playersTime = players.map((player) => player.time);

  const minTime = Math.min(...playersTime);

  const bestPlayer = players.filter((player) => player.time === minTime);
